...
SETS = {
    ...
    # 🔴 2026-09-23: **หน่วยของ SLOW_TRADE_DAYS เปลี่ยนเป็น "วันทำการ" เมื่อ 2026-09-22**
    # (ดู exit_monitor.market_days_held — หักเสาร์-อาทิตย์ให้ symbol ที่ไม่ใช่ CRYPTO)
    # ตาราง ΔR เก่าของชุดนี้วัดบน**วันปฏิทิน**ทั้งหมด **เทียบกับของใหม่ตรงๆ ไม่ได้**
    # 3 วันทำการ ~= 4.2 วันปฏิทินสำหรับ non-crypto ตัวเลือกข้างเคียงจึงเลื่อนตามไปหมด
    # backtest_trade_sim เรียก em.analyze_position จึงรับหน่วยใหม่อัตโนมัติ ไม่ต้องแก้อะไร
    # ⚠️ ชุดนี้คัดกรองได้เพราะ SLOW_TRADE_KEEP = 50 ไม่คืนช่องถือไม้ -> ชุดไม้คงที่
    #    (ยืนยันแล้ว 2026-09-22: เปลี่ยนหน่วยวันแล้วชุดไม้ไม่ขยับเลยสักไม้ 187/187)
    #    ถ้าจะกวาด SLOW_TRADE_KEEP = 0 ด้วย **ใช้ชุดนี้ไม่ได้** ต้อง replay เต็ม
    "slow": {
        "control": LIVE_SLOW,            # 3 วันทำการ = ค่าปัจจุบัน
        "2 วัน":   {"SLOW_TRADE_DAYS": 2},   # ต่ำกว่าปัจจุบัน — ไม่เคยทดสอบเลยทั้งสองหน่วย
        "4 วัน":   {"SLOW_TRADE_DAYS": 4},
    ...
    # เพดานการขยาย SL (exit_monitor.MAX_SL_WIDEN_R — 1.5 ตอนกวาด ตอนนี้ 1.2) ตัวเดียวในระบบ
    # ที่ยอมให้ขาดทุนจริง
    ...
    # ครึ่งที่สองของกฎ 1R: เลื่อน SL ไป breakeven (checklist ข้อ 5) ซึ่งไม่เคยวัดแยกเลย
    # ตอนปิด RULE_1R_KEEP (2026-09-13) ปิดไปแค่ครึ่ง "ตัดไม้ 50%" ครึ่งนี้ยังเดินอยู่ และเป็น
    # ที่มาของช่อง BE ทั้งหมด (44 ไม้ MFE เฉลี่ย 1.66R จบรวม +4.3R)
    # ⚠️ ปิด BE = ไม้ที่เคยจบ 0 จะไปได้ทั้งสองทาง (วิ่งต่อ หรือ กลับไปโดน SL เต็ม −1R)
    # MFE เป็นเพดาน ไม่ใช่กำไรที่รออยู่ — ต้องอ่านช่อง "แย่ลง" คู่กับ "ดีขึ้น" เสมอ
    "be": {
    ...
    # 🔴 2026-09-22: **ชุดนี้ใช้ตัดสินแกน belevel ไม่ได้ และผลเก่าที่เคยได้จากมันทิ้งแล้ว**
    # การย้ายจุดล็อกเปลี่ยนว่าไม้จบเมื่อไหร่ = ครองช่องถือไม้นานขึ้น/สั้นลง = **ชุดไม้เปลี่ยน**
    # ซึ่งเป็นสิ่งเดียวที่เครื่องมือตัวนี้มองไม่เห็นตามนิยามของมัน (กติกาข้อ 4 ของ CLAUDE.md)
    # วัดใหม่ด้วย replay เต็มแล้ว: `backtest_replay.py <SYM> 730 --be-level=X`
    # ฝั่งบวกแพ้ทุกช่อง (+0.2R -8.56R · +0.5R -13.31R) และ **แกนนี้ปิดจบทั้งสองทิศแล้ว**
    # ดูตัวเลขเต็มที่ exit_monitor.BREAKEVEN_TRIGGER_R — เก็บชุดนี้ไว้เฉยๆ อย่าเอาไปตัดสินอะไร
    "belevel": {
