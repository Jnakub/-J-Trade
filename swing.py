...
#   XAUUSDm 1.6x  (2026-08-19 — ปรับขึ้นจาก 1.5 ตามคำสั่งผู้ใช้ หลังพบว่าตั้งแต่ 17 ก.ค. ไม่มี
#                  Swing Low ใหม่เลยทั้งที่ราคาไต่ขึ้นต่อเนื่อง ไล่ทุกจุด fractal low ในช่วงนั้น
#                  15 จุดไม่ผ่านเกณฑ์เดิม (1.5x/0.5) สักจุด — จุดที่ใกล้ผ่านสุดคือ 08-18 22:00
#                  wick=0.47 ยังไม่มี backtest ยืนยันค่าใหม่นี้โดยเฉพาะ)
...
VOL_MULTIPLIER_BY_SYMBOL = {
    # 2026-09-19: HK50m — **ผู้ใช้เลือก 1.5/0.45 หลังดูตาราง swing 2 preset**
...
#   XAUUSDm 0.3   (2026-07-21 เริ่มที่ 0.5 — ไม่มี real volume ให้ merge, COMEX ผ่าน yfinance
#                  พังที่ 4H — ดู binance.py) wick เป็นสัญญาณอิสระที่ช่วยกู้ swing point ที่
#                  volume มองไม่เห็น (backtest 400 แท่ง: vol อย่างเดียวเจอ lows แค่ 9 จุด,
#                  เพิ่ม wick OR เจอ 19 จุด)
#                  2026-08-19: ลดจาก 0.5 เป็น 0.3 ตามคำสั่งผู้ใช้ — พบว่าตั้งแต่ 17 ก.ค. ไม่มี
#                  Swing Low ใหม่เลยทั้งที่ราคาไต่ขึ้นต่อเนื่อง ไล่ทุกจุด fractal low ในช่วงนั้น
#                  15 จุดไม่ผ่านเกณฑ์เดิม (1.5x/0.5) สักจุด ใกล้ผ่านสุดคือ wick=0.47 ยังไม่มี
#                  backtest ยืนยันค่าใหม่นี้โดยเฉพาะ ควรเฝ้าดูผลจริง
