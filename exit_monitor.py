...
#
# 🔴 2026-09-14 — **วัดค่าเสียโอกาสของการถือช่องโดยตรงแล้ว: แทบไม่มี** (backtest_replay
# --log-blocked + backtest_blocked_value.py ครบ 7 symbol 730 วัน) วิธี: ทุกรอบที่ระบบข้ามเพราะ
# "ช่องไม่ว่าง" ให้เดินด่านที่เหลือต่อจนสุด ถ้าผ่านครบก็เก็บเป็นสัญญาณเงา แล้วเดินไม้เงาต่อคิวกัน
#   ไม้เงาที่ได้ 172 ไม้ +30.36R  **แต่ 148 ไม้ (86%) เกิดภายใน 4 ชั่วโมงแรกหลังเปิดไม้เดิม**
#   เกิดหลังไม้เดิมเปิดไปแล้วเกิน 3 วัน: **7 ไม้จาก 172 (4.1%) = +2.57R ตลอด 2 ปี ทั้ง 7 symbol**
#   (~+0.18R/symbol/ปี) และ 4 ใน 7 symbol ได้ศูนย์ไม้ (BTC/ETH/USDJPY/EUR)
# => สัญญาณที่ถูกบล็อกไม่ใช่ "setup ใหม่ที่มารอคิวตอนไม้เก่าไม่ไปไหน" แต่เป็นสัญญาณเดิมที่ยังยืน
#    อยู่อีก 1-2 ชั่วโมง = การเข้าซ้ำใน move เดียวกัน ซึ่งเป็นคำถาม sizing ไม่ใช่คำถาม exit
#    **การออกช้าจึงแทบไม่มีต้นทุนค่าเสียโอกาส** — เหตุผลตั้งต้นของกฎนี้ (และของทุกกฎที่อ้างว่า
#    "ออกเร็วขึ้นเพื่อคืนช่อง") ไม่มีอยู่จริงในข้อมูล ที่ยังเหลือให้มันทำได้คือลดความเสี่ยงเฉย ๆ
SLOW_TRADE_DAYS       = 3
SLOW_TRADE_R          = 0.5
