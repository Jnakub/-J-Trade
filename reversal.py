...
from config import (MT5_TIMEFRAMES, RISK_PER_TRADE, MIN_RR_HARD_BLOCK, MAX_RR_HARD_BLOCK,
                    MIN_SL_DISTANCE_PCT, MAX_TP_DISTANCE_PCT)
...
def compute_reversal_score(symbol: str, direction: str, entry: float,
    ...
    if tp is None:
        ...
        if fib_info.get("passed"):
            tp = fib_info["levels"]["1.618"]   # 2026-08-02: เปลี่ยนจาก 0.786 -> 1.618 (ดู scoring.py
                                              # comment เดียวกัน — ปรับพร้อมกันทั้ง 2 scorecard)
        else:
