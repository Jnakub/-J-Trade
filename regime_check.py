...
# 🔻 สถานะปัจจุบัน: DIV_MIN_SPACING_BARS = 5 (กวาดแล้ว 2026-09-23 · คงไว้ ดูบล็อกที่ค่านั้น) ·
...
# และ DIV_MIN_SPACING_BARS (กวาด 3/5/8/10 เมื่อ 2026-09-23 ดูบล็อกที่ค่านั้น) — **ไม่เหลือ TODO แล้ว**
#
...
# ✅ **กวาดแล้ว 2026-09-23 (replay เต็ม 8 symbol 730 วัน) — คงไว้ที่ 5**
# TODO ที่ค้างตั้งแต่ 2026-07-25 ไม่เคยมีใครทำเพราะ **ไม่มีธงให้กวาด** — สร้าง --div-min-spacing แล้ว
#   spacing   ไม้   TotalR     ΔR   ไม้ใหม่  ไม้หาย   |t|    (identity 5 ผ่านเป๊ะ)
#       3    173   +85.76   -1.78       8       3   0.46
#     **5**  168   +87.54    0.00       -       -    -    <- ยอดของแกน
#       8    157   +81.66   -5.88       4      15   1.16
#      10    155   +78.60   -8.94       7      20   1.48
# 🔴 **direct = 0 ทุก arm = churn ล้วน** ตามที่คาดไว้ — ปุ่มนี้ **สลับคู่** ไม่ใช่ subset แท้
#    (ต่างจาก DIV_MAX_LOOKBACK_BARS) อ่านตามกติกาข้อ 3b = ดูไม้ที่หาย/ที่เข้ามาแทน:
#      spacing 3   หาย 3 ไม้ Σ+4.53 ชนะ 3/3    -> ใหม่ 8 ไม้ Σ+2.76 ชนะ 4/8
#      spacing 8   หาย 15 ไม้ Σ+9.21 ชนะ 11/15 -> ใหม่ 4 ไม้ Σ+3.33 ชนะ 4/4
#      spacing 10  หาย 20 ไม้ Σ+9.03 ชนะ 13/20 -> ใหม่ 7 ไม้ Σ+0.08 ชนะ 4/7
#    **ทั้งสองทิศเอาไม้ดีออกแล้วเอาไม้ที่แย่กว่ามาแทน** · ที่ 8/10 ไม้ที่หายเป็น Reversal/Breakout
#    เกือบหมด (Scoring แค่ 2 ไม้ทุกระดับ) สมเหตุผลเพราะสองกลยุทธ์นั้นพึ่ง divergence
# ⚠️ หลักฐานอ่อน: |t| ไม่ผ่านสักตัว · บวก 2/8 ทุกระดับ · ที่ 8 ตัด 5 ไม้ใหญ่เหลือ -0.02R
#    สิ่งที่ยืนยันได้คือ "ไม่มีเหตุผลให้ขยับ" ไม่ใช่ "5 ดีที่สุด"
# ⚠️ ค่า 5 ถูกตั้งตอนเพดานยัง 180 (หน้าต่าง [5,180]) · ตอนนี้ [5,20] พื้นกิน 1/4 ของหน้าต่าง
