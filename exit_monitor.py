...
#
# 2026-09-13 — วัดครบ 7 symbol 730 วันแล้ว (backtest_exit_rules.py --set=trend บนฐานหลังปิดกฎ
# 1R/halfway): **ไม่แตะ ปล่อยไว้ตามเดิม** เพราะกฎนี้แทบไม่มีตัวตนในทางปฏิบัติ
#   ปิดทั้งกฎ: BTC −0.30 · XAU 0.00 · ETH +1.95 · USDJPY 0.00 · US500 −0.30 · GBP −0.70 ·
#              EUR −0.03  = **+0.62R จากไม้ที่มันเคยแตะแค่ 8 ไม้ใน 177 ไม้ Scoring** บวก 1/7
#   3 ใน 7 symbol กฎไม่เคยยิงเลยสักครั้งใน 2 ปี และไม้ 8 ไม้ที่แตะก็ไม่ได้ชี้ไปทางเดียวกัน
#   (ETH ดีขึ้น 2 / BTC-US500-GBP-EUR แย่ลงทุกไม้) = ขนาด sample ที่เคยหลอกโปรเจกต์นี้มาแล้ว
# 🔴 **ขา 3 แท่ง (keep 0 = ออก 100%) ไม่เคยยิงเลยแม้แต่ครั้งเดียวทั้ง 7 symbol** — วัดโดยตรงด้วย
# รอบ "เหลือแต่ระดับ 100%" ที่ได้ +0.00R ไม้เปลี่ยน 0 ทุก symbol  ที่มันยิงจริงคือขา 1-2 แท่ง
# (ตัด 25%/50%) เท่านั้น  กลไก: grace 5 แท่ง Daily + ต้องปิดสวนติดกันอีก 3 แท่ง ~ 8 วัน ขณะที่
# ไม้ Scoring ถือเฉลี่ย ~6 วัน — ไม้ส่วนใหญ่ปิดไปก่อนกฎจะมีสิทธิ์ยิงด้วยซ้ำ
# => ความแรงต่อครั้งของกฎไม่สำคัญเท่าความถี่ที่มันได้ยิง อย่าจัดลำดับความสำคัญจาก "ตัดกี่ %"
