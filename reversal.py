...
#
# 2026-09-13 (ตัดสินใจ): **ตั้งเป็น 0 = เลิกใช้สกอร์การ์ดกรอง** ตามคำสั่งผู้ใช้ ให้ตรงกับ
# ฝั่ง Scoring ที่ปิดไปแล้ว (config.MIN_SCORE = 0.0, commit 4457efc)
# เหตุผลคือ "ด่านนี้ไม่มีอำนาจแยกแยะ" ตามตัวเลขข้างบน **ไม่ใช่เพราะมันทำเงินเพิ่ม** —
# ผลรวมทั้งพอร์ต 7 symbol: 196 ไม้ +38.23R -> 202 ไม้ +36.95R (ΔR -1.29)
#   Scoring   ไม่ขยับเลย 177 ไม้ +23.70R  (ด่านนี้ไม่แตะฝั่ง Scoring)
#   Reversal  19 ไม้ +14.53R avgR +0.765  ->  25 ไม้ +13.25R avgR +0.530
# ยอมรับ -1.29R แลกกับการเอากลไกที่พิสูจน์แล้วว่าไม่ทำงานออกจากเส้นทางเข้าไม้
#
# 🔴 สิ่งที่ต้องเฝ้าดู: ไม้ Reversal เพิ่มจาก 19 -> 25 ไม้ (+32%) แต่ avgR ตกจาก +0.765
#    เหลือ +0.530 และ t ตกจาก 3.49 -> 2.71 — ยังผ่านเกณฑ์นัยสำคัญอยู่ แต่บางลง
#    ถ้าผลจริงข้างหน้าออกมาแย่กว่านี้ ตัวนี้คือตัวแรกที่ควรถอยกลับ (ตั้งกลับเป็น 7)
