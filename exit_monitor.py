...
# ✅ **วัดซ้ำ 2026-09-24 บน base ปัจจุบัน (168 ไม้ +87.54R) — คงไว้ที่ 30 แต่เหตุผลเปลี่ยนแล้ว**
#   เพดาน    ไม้   TotalR     ΔR   direct                  churn     (identity 30 ผ่านเป๊ะ)
#   20 วัน   171   +82.69   -4.85  24 ไม้ -4.17 |t| 0.81    -0.68
#   30 วัน   168   +87.54    0.00  <- ค่าปัจจุบัน
#   45 วัน   169   +87.59   +0.05  12 ไม้ -1.34 |t| 0.40    +1.39
#   ปิด      169   +88.81   +1.27  12 ไม้ -0.09 |t| 0.02    +1.36   (ไม้ถือนานสุด 50.2 วัน)
# 🔴 **ตัวเลข "+1.86R" ด้านบนไม่จริงแล้วบน base ปัจจุบัน** — ปิดเพดาน direct = -0.09R = ศูนย์
#    +1.27R ของแถว "ปิด" มาจาก churn ล้วน ซึ่งแกนนี้เป็น "จังหวะคืนช่อง" (กติกาข้อ 3b) = noise
#    ไม้ 12 ไม้ที่ชนเพดาน ถ้าปล่อยวิ่งต่อ: **ไปถึง TP 6 ไม้ · ย้อนจบ SL/BE 6 ไม้ = โยนเหรียญ**
# 👉 "ห้ามลด" ยังจริง (20 วันเสีย -4.85R ปิดไม้ที่กำลังทำงาน) แต่เหตุผลที่คงไว้ไม่ใช่ R อีกแล้ว
#    คือ **ฟรีและจำกัดเวลาที่เงินทุนถูกล็อก** — ไม่มีเพดาน ไม้ค้างได้ถึง 50 วันโดยผลเฉลี่ยเท่าเดิม
MAX_HOLD_DAYS         = 30
